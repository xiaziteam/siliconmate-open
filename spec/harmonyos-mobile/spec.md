...
硅侣鸿蒙手机版是硅侣2.0桌面端的移动延伸，采用"云窗口"薄客户端架构——鸿蒙App只负责UI交互和网络通信，所有Agent能力（free-code + GLM + OfficeCLI）运行在鬼子虾2号VPS上。手机端通过HTTPS API与VPS通信，实现文字聊天、图片上传OCR、Office文件处理、语音输入和ChatGPT Web语音聊天透传。复用现有account服务(64.83.45.235:8444)的账号体系。
...
- **FR-006**: App MUST复用现有account服务(64.83.45.235:8444)进行登录认证
...
- 鬼子虾2号VPS(64.83.45.235)运行free-code + zhipu-bridge + OfficeCLI，通过HTTPS API对外服务
...
- 现有account服务(64.83.45.235:8444)可从手机端HTTPS访问
